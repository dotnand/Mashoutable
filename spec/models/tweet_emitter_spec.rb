...
describe TweetEmitter do
  ...
  it 'should not post to twitter if length > 140 characters' do
    content = 'a' * 200
    error   = 'Status is over 140 characters.'
    subject = build_emitter(params.merge!({ 'out' => content }))

    user.should_receive(:twitter).and_return(twitter)
    twitter.should_receive(:update).with(content).and_raise(Twitter::Error::Forbidden.new(error, {}))

    subject.twitter_post
    subject.out.out_errors.map(&:error).should eq([error])
  end
